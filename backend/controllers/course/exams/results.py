...
class results_controller():
    def get_student_results(self, student_id, exam_id):
        try:
            return Results.query.filter_by(student_id=student_id, exam_id=exam_id).first().serialize()
    ...
    def update_result(self,student_id,exam_id,new_result):
        try:
            to_be_updated=Results.query.filter_by(student_id=student_id,exam_id=exam_id).first()
