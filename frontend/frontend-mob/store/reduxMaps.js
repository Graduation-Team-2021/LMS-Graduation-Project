...
export const mapDispatchToProps = (dispatch) => {
  return {
    userDataActions: {
      onSetToken: (newToken) =>
        dispatch(userDataActionFunctions.setToken(newToken)),
      tokenError: () => dispatch(userDataActionFunctions.setToken(null)),
      onSetName: (newName) =>
        dispatch(userDataActionFunctions.setName(newName)),
    ...
    currentCoursesActions: {
      onSetCurrentCourses: (newCourses) =>
        dispatch(currentCoursesActionFunctions.setRecentEvent(newCourses)),
    },
    currentGroupsActions: {
      onSetCurrentGroups: (newGroups) =>
        dispatch(currentGroupsActionFunctions.setCurrentGroups(newGroups)),
    },
    recentEventsActions: {
      onSetRecentEvents: (newEvents) =>
        dispatch(recentEventActionFunctions.setRecentEvent(newEvents)),
    },
    finishedCoursesActions: {
      onSetFinshedCourses: (newCourses) =>
        dispatch(finishedCoursesActionFunctions.setFinishedCourses(newCourses)),
    },
    recentUserPostsActions: {
      onSetRecentUserPosts: (newUserPosts) =>
        dispatch(
          recentUserPostsActionsFunctions.setUserRecentPosts(newUserPosts)
        ),
    },
  };
};
