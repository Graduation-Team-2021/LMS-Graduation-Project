  ...
  return (
    <View style={{ flex: 1, justifyContent: "center" }}>
      ...
      <Text style={{ padding: 10, fontSize: 16, textAlign: "center" }}>
        {(Deliverable.type==="Quiz")?(`Allowed time: ${Deliverable.leeway}`):null}
      </Text>
      <Button
        //title= "go to $`props.CourseName`"
        title={(Deliverable.type==="Quiz")?(`Begin ${Deliverable.name} ${Deliverable.type}`):(`Go to ${Deliverable.name} ${Deliverable.type}`)}
        color="#F0F8FF"
        /*onPress={() => {
          props.navigation.navigate({
            routeName: "Course",
            params: { deliverableName: Deliverable.name, groupflag: false },
          });
        }}*/
      />
    </View>
